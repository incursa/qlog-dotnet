...
param(
    ...
    [string]$FirstReleaseVersion = "1.0.6",
...
foreach ($project in $projects) {
    ...
    if ($priorTag) {
        ...
        if (Test-Path $unshippedPath) {
            ...
            if ($relativeUnshipped.StartsWith($repoRoot, [StringComparison]::OrdinalIgnoreCase)) {
        ...
        if ($currentUnshipped -ne $null -and ($priorUnshippedText -ne $currentUnshipped)) {
            $unshippedWarnings.Add("$projectName => unshipped API baselines changed since $priorTag")
        }
    }
}
...
if (-not ($releaseVersion -ge [version]$FirstReleaseVersion)) {
    Write-Error "Release tag '$releaseVersion' must be at least $FirstReleaseVersion."
    exit 1
}
