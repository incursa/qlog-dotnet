...
function Get-ChangedPathFacts {
    ...
    foreach ($path in $normalized) {
        ...
        elseif (
            $path.StartsWith('specs/requirements/', [System.StringComparison]::OrdinalIgnoreCase) -or
            $path.StartsWith('specs/architecture/', [System.StringComparison]::OrdinalIgnoreCase) -or
            $path.StartsWith('specs/work-items/', [System.StringComparison]::OrdinalIgnoreCase) -or
            $path.StartsWith('specs/verification/', [System.StringComparison]::OrdinalIgnoreCase)
        ) {
...
try {
    ...
    elseif (($StopOnBlockedGap -or $BlockingGapIds.Count -gt 0) -and (Test-Path -LiteralPath (Join-Path $workRoot "specs/requirements/qlog/REQUIREMENT-GAPS.md"))) {
        $resolvedRequirementGapsPath = Resolve-ExistingPath -Path (Join-Path $workRoot "specs/requirements/qlog/REQUIREMENT-GAPS.md")
    }
